...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  const handleDiscordLogin = () => {
    setLoading(true);
    setError(null);
    // Redirect to Discord OAuth - after callback, user returns here
    window.location.href = authApi.getDiscordLoginUrl();
  };

  return (
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      {/* The switcher sits above the card rather than inside it: someone who
          landed in the wrong language needs to find it before reading
          anything, and this is the only screen shown before sign-in. */}

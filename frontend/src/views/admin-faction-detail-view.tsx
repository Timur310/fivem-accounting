...
export function AdminFactionDetailView() {
  ...
  if (!data) {
    return (<div className="text-center py-12 text-zinc-500"><p>Faction not found.</p></div>);
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-[11px] text-zinc-600 tabular-nums">Created {new Date(data.createdAt).toLocaleDateString()}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" size="sm" onClick={() => { setSelectedFactionId(data.id); setCurrentView('dashboard'); }}>
          <Settings className="mr-1.5 h-4 w-4" />Open Dashboard
        </Button>
        ...
        <Button variant="outline" size="sm" onClick={() => { setSelectedFactionId(data.id); setCurrentView('settings'); }}>
          <Package className="mr-1.5 h-4 w-4" />Edit Item Types
        </Button>
      </div>

      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm text-zinc-200 flex items-center gap-2"><Users className="h-4 w-4 text-zinc-400" />Members ({data.members.length})</CardTitle></CardHeader>
          <CardContent>
            {data.members.length === 0 ? (<p className="text-zinc-600 text-sm text-center py-6">No members.</p>) : (
        ...
        <Card>
          <CardHeader className="pb-3"><CardTitle className="text-sm text-zinc-200 flex items-center gap-2"><Package className="h-4 w-4 text-zinc-400" />Item Types ({data.itemTypes.length})</CardTitle></CardHeader>
          <CardContent>
            {data.itemTypes.length === 0 ? (<p className="text-zinc-600 text-sm text-center py-6">No item types.</p>) : (
              <div className="space-y-1 max-h-96 overflow-y-auto">
                {data.itemTypes.map((t: ItemType) => (
                  <div key={t.id} className="flex items-center justify-between py-2 px-2 -mx-2 rounded-md hover:bg-white/[0.02] transition-colors duration-100">
                    <div><p className="text-sm text-zinc-300">{t.name}</p><p className="text-[11px] text-zinc-600">Unit: {t.unit}</p></div>
                    <span className={`text-[11px] px-2 py-0.5 rounded-md font-medium border ${t.isActive ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-white/[0.04] text-zinc-500 border-white/[0.06]'}`}>{t.isActive ? 'Active' : 'Disabled'}</span>
                  </div>
                ))}
              </div>
            )}
